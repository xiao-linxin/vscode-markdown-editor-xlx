...
- **新增：复制 AI 引用**。选中若干行按 `Ctrl+Shift+C`（Mac `Cmd+Shift+C`）即可把 `@相对路径:行号`（多行为 `@相对路径:起始-结束`）复制到剪贴板，粘进 AI 对话就能定位到具体行；未选中时复制光标所在行。
  - 编辑器右键菜单「复制/粘贴」区域新增「复制引用」入口。
  - 新增配置 `copyReferencePrefix`（默认 `@`）、`copyReferencePathSeparator`（默认 `:`）、`copyReferenceLineSeparator`（默认 `-`）。
  - 仅写剪贴板，**不会**自动聚焦或粘贴到任何 AI 面板。
  - 对所有文件类型生效（不限于 Markdown）。
